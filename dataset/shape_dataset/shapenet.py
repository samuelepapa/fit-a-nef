...
class ShapeNet(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if np.isnan(points).any():
            raise ValueError(
                idx, "nan: ", self.root, self.models[idx]["category"], self.models[idx]["model"]
            )
            print(
                idx, ". nan: ", self.root, self.models[idx]["category"], self.models[idx]["model"]
            )

        if np.isnan(occupancies).any():
            raise ValueError(
                idx, "nan: ", self.root, self.models[idx]["category"], self.models[idx]["model"]
            )
            print(
...
if __name__ == "__main__":
    train_dataset = ShapeNet(
        root="/media/davidknigge/hard-disk1/storage/ShapeNet", num_points=(1024, 1024)
    )
